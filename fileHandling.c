...
int main() {
    ...
    int source_fd = open("source.txt", O_RDWR | O_CREAT, 0644); // O_RDWR allows both reading and writing
    if (source_fd == -1) {
    ...
    char content[50];
    printf("Enter the text to be written in source file");
    fgets(content,sizeof(content),stdin);
    int sz = write(source_fd,content, strlen(content));
    printf("%d\n", sz);
